...
| 04 | [04-bootstrap-apps.sh](04-bootstrap-apps.sh) | Register `ARGOCD_GIT_USER` + `ARGOCD_GIT_PAT` with Argo CD via a temporary port-forward (so the root AppSet can list a private repo), then `kubectl apply` the root ApplicationSet — Argo CD now owns the cluster |
...
| 09 | [09-host-apps.sh](09-host-apps.sh) | Reconcile `host/apps/*` (Docker compose containers on Colima) and splice their scrape targets into the Prometheus appset |
| 10 | [10-mac-exporter.sh](10-mac-exporter.sh) | Install [node_exporter](https://github.com/prometheus/node_exporter) (darwin build, via brew) and [mac-extras.sh](mac-extras.sh) as LaunchDaemons. Binds node_exporter to the Colima vmnet bridge IP so only the cluster (not the LAN) can scrape it. Splices a `mac-host` scrape job into `cluster/apps/prometheus/_appset.yaml` |
| 11 | [11-mdns-reflector.sh](11-mdns-reflector.sh) | Install [mdns-reflector.py](mdns-reflector.py) as a LaunchDaemon. Browses `_hap._tcp` on the Colima vmnet bridge (where HA's HomeKit Bridge in `network_mode: host` announces) and re-publishes on the Mac's LAN interface with A records rewritten to the Mac's LAN IP. Required for Apple TV in the iot VLAN to discover HA's HomeKit Bridge -- the TCP path is already handled by Colima's host-listener tunnel + the MikroTik forward rule that opens `iot -> 192.168.10.3:21063` |
...
| `ARGOCD_GIT_USER` / `ARGOCD_GIT_PAT` | yes (if repo is private) | GitHub username + fine-grained PAT (Contents = read-only on this repo). Step 04 registers them with Argo CD so the root AppSet can list `cluster/apps/*` |
| `GRAFANA_ADMIN_PASSWORD` | no | If unset, step 06 auto-generates one and prints it once |
| `GRAFANA_SLACK_BOT_TOKEN` | no | If set, step 06 creates `monitoring/grafana-slack` for alert contact points |
| `HOMEASSISTANT_PROMETHEUS_TOKEN` | no | If set, step 06 creates the secret Prometheus mounts to scrape HA |
...
Everything Argo CD apps need is generated by `06-cluster-secrets.sh` from
values in `bootstrap/.env`. The git repo PAT used to be a manual step; it
is now handled by `04-bootstrap-apps.sh` (reads `ARGOCD_GIT_USER` /
`ARGOCD_GIT_PAT` and runs `argocd repo add --upsert`).

Before a teardown / `colima delete`, run:

```bash
./bootstrap/dump-cluster-secrets.sh
```

This reads every cluster-side secret (including the git PAT and the
Argo CD admin password is intentionally NOT captured — it regenerates
on a fresh install) and upserts the corresponding KEY=value back into
`bootstrap/.env`, so the next `./bootstrap/bootstrap.sh` recreates the
exact same secrets and no manual re-credentialing is needed.
